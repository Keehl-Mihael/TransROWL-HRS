...
void resource2ID()
{
    ...
    ifstream file(datapath + "resource2extract_inverse.txt");
    ifstream filedb(datapath + "OTTIMO.txt");
    ofstream out_trip(datapath + "triples_extracted_OTTIMO_inverse.txt");
    ...
    for(vector<string>::iterator itt = triples.begin(); itt != triples.end(); itt++){
        out_trip << (*itt) <<endl;
    }
/*
    while (getline(file, str))
