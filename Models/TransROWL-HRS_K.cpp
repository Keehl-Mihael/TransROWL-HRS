...
REAL *relationVec,*relationCVec, *entityVec, *matrix;
...
void gradient(INT e1_a, INT e2_a, INT rel_a, INT belta, INT same, REAL *tmp1, REAL *tmp2) {
	...
	for (INT ii=0; ii < dimensionR; ii++) {
		...
		relationVecDao[lastar + ii] -= same * x*(1+lambdaR);
        relationCVec[lastarc + ii] -= same * x*(1+lambdaC);
...
void gradientSubClass(INT cls1_a, INT cls2_a, INT cls1_b, INT cls2_b) {
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        if (x > 0)
            ...
            x = -belta * alpha;
        for (INT jj = 0; jj < dimension; jj++) {
            matrixDao[lastM + jj] -=  x * (entityVec[lastb1 + jj] - entityVec[lastb2 + jj]);
            entityVecDao[lastb1 + jj] -= x * matrix[lastM + jj];
            entityVecDao[lastb2 + jj] += x * matrix[lastM + jj];
        }
        //relationVecDao[lastar + ii] -= same * x;
        lastM = lastM + dimension;
    }
...
void gradientInverseOf(INT e1_a, INT e2_a, INT rel_a, INT belta, INT same, REAL *tmp1, REAL *tmp2,INT e1_b, INT e2_b, INT rel_b, INT belta2, INT same2, REAL *tmp3, REAL *tmp4, INT inverseRel) {
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        if (x2 > 0)
            ...
            x2 = -belta * alpha;
        for (INT jj = 0; jj < dimension; jj++) {
            matrixDao[lastMa + jj] -=  x * (entityVec[lasta1 + jj] - entityVec[lasta2 + jj]);
            entityVecDao[lasta1 + jj] -= x * matrix[lastMa + jj];
            entityVecDao[lasta2 + jj] += x * matrix[lastMa + jj];

            matrixDao[lastMI + jj] -=  x2 * (entityVec[lasta2 + jj] - entityVec[lasta1 + jj]);
            entityVecDao[lasta2 + jj] -= x2 * matrix[lastMI + jj];
            entityVecDao[lasta1 + jj] += x2 * matrix[lastMI + jj];
        }
        relationVecDao[lastar + ii] -= same * x*(1+lambdaR);
        relationCVec[lastarc + ii] -= same * x*(1+lambdaC);
        lastMa = lastMa + dimension;
        relationVecDao[lastInverse + ii] -= same * x2*(1+lambdaR);
        relationCVec[lastInverseC + ii] -= same * x2*(1+lambdaC);
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        if (x2 > 0)
            ...
            x2 = -belta2 * alpha;
        for (INT jj = 0; jj < dimension; jj++) {
            matrixDao[lastMb + jj] -=  x * (entityVec[lastb1 + jj] - entityVec[lastb2 + jj]);
            entityVecDao[lastb1 + jj] -= x * matrix[lastMb + jj];
            entityVecDao[lastb2 + jj] += x * matrix[lastMb + jj];

            matrixDao[lastMI + jj] -=  x2 * (entityVec[lastb2 + jj] - entityVec[lastb1 + jj]);
            entityVecDao[lastb2 + jj] -= x2 * matrix[lastMI + jj];
            entityVecDao[lastb1 + jj] += x2 * matrix[lastMI + jj];
        }
        relationVecDao[lastbr + ii] -= same2 * x*(1+lambdaR);
        relationCVec[lastbrc + ii] -= same2 * x*(1+lambdaC);
        lastMb = lastMb + dimension;
        relationVecDao[lastInverse + ii] -= same2 * x2*(1+lambdaR);
        relationCVec[lastInverseC + ii] -= same2 * x2*(1+lambdaC);
        lastMI = lastMI + dimension;
    }
...
void gradientEquivalentProperty(INT e1_a, INT e2_a, INT rel_a, INT belta, INT same, REAL *tmp1, REAL *tmp2,INT e1_b, INT e2_b, INT rel_b, INT belta2, INT same2, REAL *tmp3, REAL *tmp4, INT equivalent) {
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        for (INT jj = 0; jj < dimension; jj++) {
            ...
            matrixDao[lastME + jj] -=  x2 * (entityVec[lasta1 + jj] - entityVec[lasta2 + jj]);
            entityVecDao[lasta1 + jj] -= x2 * matrix[lastME + jj];
            entityVecDao[lasta2 + jj] += x2 * matrix[lastME + jj];
        }
        relationVecDao[lastar + ii] -= same * x*(1+lambdaR);
        relationCVec[lastarc + ii] -= same * x*(1+lambdaC);
        relationVecDao[lastEquivalent + ii] -= same * x2*(1+lambdaR);
        relationCVec[lastEquivalentC + ii] -= same * x2*(1+lambdaC);
        lastMa = lastMa + dimension;
    }
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        for (INT jj = 0; jj < dimension; jj++) {
            ...
            matrixDao[lastME + jj] -=  x2 * (entityVec[lastb1 + jj] - entityVec[lastb2 + jj]);
            entityVecDao[lastb1 + jj] -= x2 * matrix[lastME + jj];
            entityVecDao[lastb2 + jj] += x2 * matrix[lastME + jj];
        }
        relationVecDao[lastbr + ii] -= same2 * x*(1+lambdaR);
        relationCVec[lastbrc + ii] -= same2 * x*(1+lambdaC);
        relationVecDao[lastEquivalent + ii] -= same2 * x2*(1+lambdaR);
        relationCVec[lastEquivalentC + ii] -= same2 * x2*(1+lambdaC);
        lastMb = lastMb + dimension;
    }
        ...
        for(set<int>::iterator it = eq_cls.begin(); it != eq_cls.end(); ++it) {
            ...
            for (INT ii=0; ii < dimensionR; ii++) {
                ...
                if (x > 0)
                    ...
                    x = -belta * alpha*0.1;
                for (INT jj = 0; jj < dimension; jj++) {
                    matrixDao[lastMa + jj] -=  x * (entityVec[lasta1 + jj] - entityVec[lastCls + jj]);
                    entityVecDao[lasta1 + jj] -= x * matrix[lastMa + jj];
                    entityVecDao[lastCls + jj] += x * matrix[lastMa + jj];
                }
                relationVecDao[lastar + ii] -= same * x*(1+lambdaR);
                relationCVec[lastarc + ii] -= same * x*(1+lambdaC);
                lastMa = lastMa + dimension;
            ...
            if (randd(id) % 1000 < pr) {
                ...
                for (INT ii=0; ii < dimensionR; ii++) {
                    ...
                    if (x > 0)
                        ...
                        x = -belta2 * alpha*0.1;
                    for (INT jj = 0; jj < dimension; jj++) {
                        matrixDao[lastMb + jj] -=  x * (entityVec[last_head + jj] - entityVec[lastCls + jj]);
                        entityVecDao[last_head + jj] -= x * matrix[lastMb + jj];
                        entityVecDao[lastCls + jj] += x * matrix[lastMb + jj];
                    }
                    relationVecDao[lastbr + ii] -= same2 * x*(1+lambdaR);
                    relationCVec[lastbrc + ii] -= same2 * x*(1+lambdaC);
                    lastMb = lastMb + dimension;
