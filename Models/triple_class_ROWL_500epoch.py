...
file_in = open(files_src + 'entity2vec_500epoch.vec', 'r')
for line in file_in:
    x=line.split()
    sub_mat = []
    for val in x: 
...
file_in = open(files_src + 'relation2vec_500epoch.vec', 'r')
for line in file_in:
    x=line.split()
    sub_mat = []
    for val in x: 
...
file_in = open(files_src + 'A_500epoch.vec', 'r')
sub_mat = []
for line in file_in:
    x=line.split()
    for val in x: 
...
for i in range(1):
    ...
    test_file = files_src + 'false2id.txt'
    f_fast_result=[]
    f_owl_result=[]
    file_in = open(test_file, 'r')
    first_line = True   #salto prima riga, contiene numero di triple
...
with open("/home/mihaelkeehl/Documenti/Tesi/MaterialeCodice/Creazioni/TransROWL - HRS/Test_acc/" +"result_lp_500epoch.txt", "a") as myfile:
    myfile.write("tc_acc_FAST.append(" + str(np.mean(acc_f)) + ")\n")
    myfile.write("tc_prec_FAST.append(" + str(np.mean(prec_f)) + ")\n")
    myfile.write("tc_rec_FAST.append(" + str(np.mean(rec_f)) + ")\n")
    myfile.write("tc_fpr_FAST.append(" + str(np.mean(fpr_f)) + ")\n")
    
    myfile.write("tc_acc_OWL.append(" + str(np.mean(acc_o)) + ")\n")
    myfile.write("tc_prec_OWL.append(" + str(np.mean(prec_o)) + ")\n")
    myfile.write("tc_rec_OWL.append(" + str(np.mean(rec_o)) + ")\n")
    myfile.write("tc_fpr_OWL.append(" + str(np.mean(fpr_o)) + ")\n")
