...
void gradientSubClass(INT cls1_a, INT cls2_a, INT cls1_b, INT cls2_b) {
    ...
    for (INT ii=0; ii < dimension; ii++) {
        x = entityVec[lasta2 + ii] -entityVec[lasta1 + ii];
        //x = tmp2[ii] - tmp1[ii] - relationVec[lastar + ii];
        if (x > 0)
            x = belta * alpha * delta;
        else
...
void gradientInverseOf(INT e1_a, INT e2_a, INT rel_a, INT belta, INT same, REAL *tmp1, REAL *tmp2,INT e1_b, INT e2_b, INT rel_b, INT belta2, INT same2, REAL *tmp3, REAL *tmp4, INT inverseRel) {
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        x = tmp2[ii] - tmp1[ii] - relationVec[lastar + ii];
        x2 = relationVec[lastar + ii] + relationVec[lastInverse + ii];
        if (x > 0)
            x = belta * alpha;
        else
            x = -belta * alpha;
        if (x2 > 0)
            x2 = -belta * alpha * delta;
        else
            x2 = belta * alpha * delta;
        for (INT jj = 0; jj < dimension; jj++) {
            x3 = matrix[lastMI + jj] - matrix[lastMa + jj];
            if (x3>0)
                x3 = belta * alpha * delta;
            else
                x3 = -belta * alpha * delta;
            matrixDao[lastMa + jj] -=  x * (entityVec[lasta1 + jj] - entityVec[lasta2 + jj]) ;
            matrixDao[lastMa + jj] -= x3;
            matrixDao[lastMI + jj] += x3;
            entityVecDao[lasta1 + jj] -= x * matrix[lastMa + jj];
            entityVecDao[lasta2 + jj] += x * matrix[lastMa + jj];
        }
        relationVecDao[lastar + ii] -= same * x;
        relationVecDao[lastar + ii] -= same * x2;
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        x2 = relationVec[lastbr + ii] + relationVec[lastInverse + ii];
        if (x > 0)
            x = belta2 * alpha;
        else
            x = -belta2 * alpha;
...
void gradientEquivalentProperty(INT e1_a, INT e2_a, INT rel_a, INT belta, INT same, REAL *tmp1, REAL *tmp2,INT e1_b, INT e2_b, INT rel_b, INT belta2, INT same2, REAL *tmp3, REAL *tmp4, INT equivalent) {
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        x = tmp2[ii] - tmp1[ii] - relationVec[lastar + ii];
        x2 = relationVec[lastEquivalent + ii] - relationVecDao[lastar + ii];
        if (x > 0)
            x = belta * alpha;
        else
            x = -belta * alpha;
        if (x2 > 0)
            ...
            x2 = -belta * alpha * delta;
        for (INT jj = 0; jj < dimension; jj++) {
            x3 = matrix[lastME + jj] - matrix[lastMa + jj] ;
            if (x3>0)
                x3 = belta * alpha * delta;
            else
                x3 = -belta * alpha * delta;
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        x = tmp4[ii] - tmp3[ii] - relationVec[lastbr + ii];
        x2 = relationVec[lastEquivalent + ii] - relationVecDao[lastbr + ii];
        if (x > 0)
            x = belta2 * alpha;
        else
            x = -belta2 * alpha;
        if (x2 > 0)
            ...
            x2 = -belta * alpha * delta;
        for (INT jj = 0; jj < dimension; jj++) {
            x3 = matrix[lastME + jj] - matrix[lastMb + jj];
            if (x3>0)
                x3 = belta * alpha * delta;
            else
                x3 = -belta * alpha * delta;
        ...
        relationVecDao[lastbr + ii] -= same2 * (x);
        relationVecDao[lastbr + ii] -= same * (x2);
        relationVecDao[lastEquivalent + ii] += same2 * (x2);
        lastMb = lastMb + dimension;
        lastME = lastME + dimension;
    }
...
void gradientEquivalentClass(INT e1_a, INT e2_a, INT rel_a, INT belta, INT same, REAL *tmp1, REAL *tmp2,INT e1_b, INT e2_b, INT rel_b, INT belta2, INT same2, REAL *tmp3, REAL *tmp4, INT id) {
    ...
    if(eq_cls.size() > 0) {
        ...
        for(set<int>::iterator it = eq_cls.begin(); it != eq_cls.end(); ++it) {
            ...
            for (INT ii=0; ii < dimensionR; ii++) {
                //x = tmpp2[ii] - tmpp1[ii];
                x = entityVec[lastCls + ii] - entityVec[lasta1 + ii];
                if (x > 0)
                    x = belta * alpha * delta;
                else
                    x = -belta * alpha * delta;
...
//                }
                entityVecDao[lasta1 + ii] -= x ;
                entityVecDao[lastCls + ii] += x ;
                lastMa = lastMa + dimension;
            }
            norm(entityVecDao + dimension * (*it),dimension);
            //norm(entityVecDao + dimension * (*it), matrixDao + dimension * dimensionR * typeOf_id);
            lastMa = rel_a * dimensionR * dimension;
            i++;
        }
    }
