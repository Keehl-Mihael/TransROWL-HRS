...
string loadPath = "";
string initPath = "Dataset/";
string note = "_ROWL-HRS_Momentum";
string note1 = "_E";
...
void init() {
    ...
    for (INT i = 0; i < ((int)cluster2rel.size()); i++) {
        for (INT ii=0; ii<dimension; ii++)
            relationCVecUpdate[i * dimension + ii] = 0;
    }
    ...
    for (INT i = 0; i < relationTotal; i++) {
        for (INT ii=0; ii < dimensionR; ii++)
            relationVecUpdate[i * dimensionR + ii] = 0;
    }
    ...
    for (INT i = 0; i < entityTotal; i++) {
        for (INT ii=0; ii < dimension; ii++)
            entityVecUpdate[i * dimension + ii] = 0;
    }
	...
	matrixUpdate=(REAL *) calloc(relationTotal * dimension * dimensionR * 2, sizeof(REAL));
    for (INT i = 0; i < relationTotal; i++)
        for (INT j = 0; j < dimensionR; j++)
            for (INT k = 0; k < dimension; k++)
                matrixUpdate[i * dimension * dimensionR + j * dimension + k] = 0;
...
void gradient(INT e1_a, INT e2_a, INT rel_a, INT belta, INT same, REAL *tmp1, REAL *tmp2) {
	...
	for (INT ii=0; ii < dimensionR; ii++) {
		...
		if (x > 0)
			...
			x = -belta * alpha;
		for (INT jj = 0; jj < dimension; jj++) {
			matrixDao[lastM + jj] -=  x * (entityVec[lasta1 + jj] - entityVec[lasta2 + jj]) + rho*matrixUpdate[lastM+jj];
			matrixUpdate[lastM+jj] = x * (entityVec[lasta1 + jj] - entityVec[lasta2 + jj]) + rho*matrixUpdate[lastM+jj];
			entityVecDao[lasta1 + jj] -= x * matrix[lastM + jj] + rho*entityVecUpdate[lasta1 +jj];
			entityVecUpdate[lasta1 + jj] = x * matrix[lastM + jj] + rho*entityVecUpdate[lasta1 +jj];
			entityVecDao[lasta2 + jj] += x * matrix[lastM + jj] + rho*entityVecUpdate[lasta2 +jj];
            entityVecUpdate[lasta2 + jj] = x * matrix[lastM + jj] + rho*entityVecUpdate[lasta2 +jj];

        }
		relationVecDao[lastar + ii] -= same * x*(1+lambdaR) + rho*relationVecUpdate[lastar + ii];
		relationVecUpdate[lastar + ii] = same * x*(1+lambdaR) + rho*relationVecUpdate[lastar + ii];
        relationCVec[lastarc + ii] -= same * x*(1+lambdaC) + rho*relationCVecUpdate[lastarc + ii];
		relationCVecUpdate[lastarc + ii] = same * x*(1+lambdaC) + rho*relationCVecUpdate[lastarc + ii];
...
void gradientSubClass(INT cls1_a, INT cls2_a, INT cls1_b, INT cls2_b) {
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        if (x > 0)
            ...
            x = -belta * alpha;
        for (INT jj = 0; jj < dimension; jj++) {
            matrixDao[lastM + jj] -=  x * (entityVec[lastb1 + jj] - entityVec[lastb2 + jj]) + rho*matrixUpdate[lastM+jj];
            matrixUpdate[lastM+jj] = x * (entityVec[lastb1 + jj] - entityVec[lastb2 + jj]) + rho*matrixUpdate[lastM+jj];
            entityVecDao[lastb1 + jj] -= x * matrix[lastM + jj] + rho*entityVecUpdate[lastb1 +jj];
            entityVecUpdate[lastb1 + jj] = x * matrix[lastM + jj] + rho*entityVecUpdate[lastb1 +jj];
            entityVecDao[lastb2 + jj] += x * matrix[lastM + jj] + rho*entityVecUpdate[lastb2 +jj];
            entityVecUpdate[lastb2 + jj] = x * matrix[lastM + jj] + rho*entityVecUpdate[lastb2 +jj];
        }
        //relationVecDao[lastar + ii] -= same * x;
        lastM = lastM + dimension;
    }
...
void gradientInverseOf(INT e1_a, INT e2_a, INT rel_a, INT belta, INT same, REAL *tmp1, REAL *tmp2,INT e1_b, INT e2_b, INT rel_b, INT belta2, INT same2, REAL *tmp3, REAL *tmp4, INT inverseRel) {
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        if (x2 > 0)
            ...
            x2 = -belta * alpha;
        for (INT jj = 0; jj < dimension; jj++) {
            matrixDao[lastMa + jj] -=  x * (entityVec[lasta1 + jj] - entityVec[lasta2 + jj]) + rho*matrixUpdate[lastMa+jj];
            matrixUpdate[lastMa+jj] = x * (entityVec[lasta1 + jj] - entityVec[lasta2 + jj]) + rho*matrixUpdate[lastMa+jj];
            entityVecDao[lasta1 + jj] -= x * matrix[lastMa + jj] + rho*entityVecUpdate[lasta1 +jj];
            entityVecUpdate[lasta1 + jj] = x * matrix[lastMa + jj] + rho*entityVecUpdate[lasta1 +jj];
            entityVecDao[lasta2 + jj] += x * matrix[lastMa + jj] + rho*entityVecUpdate[lasta2 +jj];
            entityVecUpdate[lasta2 + jj] = x * matrix[lastMa + jj] + rho*entityVecUpdate[lasta2 +jj];

            matrixDao[lastMI + jj] -=  x2 * (entityVec[lasta2 + jj] - entityVec[lasta1 + jj]) + rho*matrixUpdate[lastMI+jj];
            matrixUpdate[lastMI+jj] = x2 * (entityVec[lasta2 + jj] - entityVec[lasta1 + jj]) + rho*matrixUpdate[lastMI+jj];
            entityVecDao[lasta1 + jj] += x2 * matrix[lastMI + jj] + rho*entityVecUpdate[lasta1 +jj];
            entityVecUpdate[lasta1 + jj] = x2 * matrix[lastMI + jj] + rho*entityVecUpdate[lasta1 +jj];
            entityVecDao[lasta2 + jj] -= x2 * matrix[lastMI + jj] + rho*entityVecUpdate[lasta2 +jj];
            entityVecUpdate[lasta2 + jj] = x2 * matrix[lastMI + jj] + rho*entityVecUpdate[lasta2 +jj];
        }
        relationVecDao[lastar + ii] -= same * x*(1+lambdaR) + rho*relationVecUpdate[lastar + ii];
        relationVecUpdate[lastar + ii] = same * x*(1+lambdaR) + rho*relationVecUpdate[lastar + ii];
        relationCVec[lastarc + ii] -= same * x*(1+lambdaC) + rho*relationCVecUpdate[lastarc + ii];
        ...
        relationVecDao[lastInverse + ii] -= same * x2*(1+lambdaR) + rho*relationVecUpdate[lastInverse + ii];
        relationVecUpdate[lastInverse + ii] = same * x2*(1+lambdaR) + rho*relationVecUpdate[lastInverse + ii];
        relationCVec[lastInverseC + ii] -= same * x2*(1+lambdaC) + rho*relationCVecUpdate[lastInverseC + ii];
        relationCVecUpdate[lastInverseC + ii] = same * x2*(1+lambdaC) + rho*relationCVecUpdate[lastInverseC + ii];
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        if (x2 > 0)
            ...
            x2 = -belta2 * alpha;
        for (INT jj = 0; jj < dimension; jj++) {
            matrixDao[lastMb + jj] -=  x * (entityVec[lastb1 + jj] - entityVec[lastb2 + jj]) + rho*matrixUpdate[lastMb+jj];
            matrixUpdate[lastMb+jj] = x * (entityVec[lastb1 + jj] - entityVec[lastb2 + jj]) + rho*matrixUpdate[lastMb+jj];
            entityVecDao[lastb1 + jj] -= x * matrix[lastMb + jj] + rho*entityVecUpdate[lastb1 +jj];
            entityVecUpdate[lastb1 + jj] = x * matrix[lastMb + jj] + rho*entityVecUpdate[lastb1 +jj];
            entityVecDao[lastb2 + jj] += x * matrix[lastMb + jj] + rho*entityVecUpdate[lastb2 +jj];
            entityVecUpdate[lastb2 + jj] = x * matrix[lastMb + jj] + rho*entityVecUpdate[lastb2 +jj];

            matrixDao[lastMI + jj] -=  x2 * (entityVec[lastb2 + jj] - entityVec[lastb1 + jj]) + rho*matrixUpdate[lastMI+jj];
            matrixUpdate[lastMI + jj] = x2 * (entityVec[lastb2 + jj] - entityVec[lastb1 + jj]) + rho*matrixUpdate[lastMI+jj];
            entityVecDao[lastb1 + jj] += x2 * matrix[lastMI + jj] + rho*entityVecUpdate[lastb1 +jj];
            entityVecUpdate[lastb1 + jj] = x2 * matrix[lastMI + jj] + rho*entityVecUpdate[lastb1 +jj];
            entityVecDao[lastb2 + jj] -= x2 * matrix[lastMI + jj] + rho*entityVecUpdate[lastb2 +jj];
            entityVecUpdate[lastb2 + jj] = x2 * matrix[lastMI + jj] + rho*entityVecUpdate[lastb2 +jj];
        }
        relationVecDao[lastbr + ii] -= same2 * x*(1+lambdaR) + rho*relationVecUpdate[lastbr + ii];
        relationVecUpdate[lastbr + ii] = same2 * x*(1+lambdaR) + rho*relationVecUpdate[lastbr + ii];
        relationCVec[lastbrc + ii] -= same2 * x*(1+lambdaC) + rho*relationCVecUpdate[lastbrc + ii];
        relationCVecUpdate[lastbrc + ii] = same2 * x*(1+lambdaC) + rho*relationCVecUpdate[lastbrc + ii];
        lastMb = lastMb + dimension;
        relationVecDao[lastInverse + ii] -= same2 * x2*(1+lambdaR) + rho*relationVecUpdate[lastInverse + ii];
        relationVecUpdate[lastInverse + ii] = same2 * x2*(1+lambdaR) + rho*relationVecUpdate[lastInverse + ii];
        relationCVec[lastInverseC + ii] -= same2 * x2*(1+lambdaC) + rho*relationCVecUpdate[lastInverseC + ii];
        relationCVecUpdate[lastInverseC + ii] = same2 * x2*(1+lambdaC) + rho*relationCVecUpdate[lastInverseC + ii];
        lastMI = lastMI + dimension;
    }
...
void gradientEquivalentProperty(INT e1_a, INT e2_a, INT rel_a, INT belta, INT same, REAL *tmp1, REAL *tmp2,INT e1_b, INT e2_b, INT rel_b, INT belta2, INT same2, REAL *tmp3, REAL *tmp4, INT equivalent) {
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        for (INT jj = 0; jj < dimension; jj++) {
            ...
            matrixDao[lastME + jj] -=  x2 * (entityVec[lasta1 + jj] - entityVec[lasta2 + jj]) + rho*matrixUpdate[lastME+jj];
            matrixUpdate[lastME+jj] = x2 * (entityVec[lasta1 + jj] - entityVec[lasta2 + jj]) + rho*matrixUpdate[lastME+jj];
            entityVecDao[lasta1 + jj] -= x2 * matrix[lastME + jj] + rho*entityVecUpdate[lasta1 +jj];
            entityVecUpdate[lasta1 + jj] = x2 * matrix[lastME + jj] + rho*entityVecUpdate[lasta1 +jj];
            entityVecDao[lasta2 + jj] += x2 * matrix[lastME + jj] + rho*entityVecUpdate[lasta2 +jj];
            entityVecUpdate[lasta2 + jj] = x2 * matrix[lastME + jj] + rho*entityVecUpdate[lasta2 +jj];
        }
        relationVecDao[lastar + ii] -= same * x*(1+lambdaR) + rho*relationVecUpdate[lastar + ii];
        relationVecUpdate[lastar + ii] = same * x*(1+lambdaR) + rho*relationVecUpdate[lastar + ii];
        relationCVec[lastarc + ii] -= same * x*(1+lambdaC) + rho*relationCVecUpdate[lastarc + ii];
        relationCVecUpdate[lastarc + ii] = same * x*(1+lambdaC) + rho*relationCVecUpdate[lastarc + ii];
        lastMa = lastMa + dimension;
        relationVecDao[lastEquivalent + ii] -= same * x2*(1+lambdaR) + rho*relationVecUpdate[lastEquivalent + ii];
        relationVecUpdate[lastEquivalent + ii] = same * x2*(1+lambdaR) + rho*relationVecUpdate[lastEquivalent + ii];
        relationCVec[lastEquivalentC + ii] -= same * x2*(1+lambdaC) + rho*relationCVecUpdate[lastEquivalentC + ii];
        relationCVecUpdate[lastEquivalentC + ii] = same * x2*(1+lambdaC) + rho*relationCVecUpdate[lastEquivalentC + ii];
        lastME = lastME + dimension;
    }

    lastME = equivalent * dimensionR * dimension;
    ...
    for (INT ii=0; ii < dimensionR; ii++) {
        ...
        relationCVecUpdate[lastbrc + ii] = same2 * x*(1+lambdaC) + rho*relationCVecUpdate[lastbrc + ii];
        lastMb = lastMb + dimension;
        relationVecDao[lastEquivalent + ii] -= same2 * x2*(1+lambdaR) + rho*relationVecUpdate[lastEquivalent + ii];
        relationVecUpdate[lastEquivalent + ii] = same2 * x2*(1+lambdaR) + rho*relationVecUpdate[lastEquivalent + ii];
        relationCVec[lastEquivalentC + ii] -= same2 * x2*(1+lambdaC) + rho*relationCVecUpdate[lastEquivalentC + ii];
        relationCVecUpdate[lastEquivalentC + ii] = same2 * x2*(1+lambdaC) + rho*relationCVecUpdate[lastEquivalentC + ii];
        lastME = lastME + dimension;
    }
        ...
        for(set<int>::iterator it = eq_cls.begin(); it != eq_cls.end(); ++it) {
            ...
            for (INT ii=0; ii < dimensionR; ii++) {
                ...
                if (x > 0)
                    ...
                    x = -belta * alpha*0.1;
                for (INT jj = 0; jj < dimension; jj++) {
                    matrixDao[lastMa + jj] -=  x * (entityVec[lasta1 + jj] - entityVec[lastCls + jj]) + rho*matrixUpdate[lastMa+jj];
                    matrixUpdate[lastMa+jj] = x * (entityVec[lasta1 + jj] - entityVec[lastCls + jj]) + rho*matrixUpdate[lastMa+jj];
                    entityVecDao[lasta1 + jj] -= x * matrix[lastMa + jj] + rho*entityVecUpdate[lasta1 +jj];
                    entityVecUpdate[lasta1 + jj] = x * matrix[lastMa + jj] + rho*entityVecUpdate[lasta1 +jj];
                    entityVecDao[lastCls + jj] += x * matrix[lastMa + jj] + rho*entityVecUpdate[lastCls +jj];
                    entityVecUpdate[lastCls + jj] = x * matrix[lastMa + jj] + rho*entityVecUpdate[lastCls +jj];
                }
                relationVecDao[lastar + ii] -= same * x*(1+lambdaR) + rho*relationVecUpdate[lastar + ii];
                relationVecUpdate[lastar + ii] = same * x*(1+lambdaR) + rho*relationVecUpdate[lastar + ii];
                relationCVec[lastarc + ii] -= same * x*(1+lambdaC) + rho*relationCVecUpdate[lastarc + ii];
            ...
            if (randd(id) % 1000 < pr) {
                ...
                for (INT ii=0; ii < dimensionR; ii++) {
                    ...
                    if (x > 0)
                        ...
                        x = -belta2 * alpha*0.1;
                    for (INT jj = 0; jj < dimension; jj++) {
                        matrixDao[lastMb + jj] -=  x * (entityVec[lasta1 + jj] - entityVec[last_tail + jj]);
                        entityVecDao[lasta1 + jj] -= x * matrix[lastMb + jj];
                        entityVecDao[last_tail + jj] += x * matrix[lastMb + jj];

                        matrixDao[lastMb + jj] -=  x * (entityVec[lasta1 + jj] - entityVec[last_tail + jj]) + rho*matrixUpdate[lastMb+jj];
                        matrixUpdate[lastMb+jj] = x * (entityVec[lasta1 + jj] - entityVec[last_tail + jj]) + rho*matrixUpdate[lastMb+jj];
                        entityVecDao[lasta1 + jj] -= x * matrix[lastMb + jj] + rho*entityVecUpdate[lasta1 +jj];
                        entityVecUpdate[lasta1 + jj] = x * matrix[lastMb + jj] + rho*entityVecUpdate[lasta1 +jj];
                        entityVecDao[last_tail + jj] += x * matrix[lastMb + jj] + rho*entityVecUpdate[last_tail +jj];
                        entityVecUpdate[last_tail + jj] = x * matrix[lastMb + jj] + rho*entityVecUpdate[last_tail +jj];
                    }
                    relationVecDao[lastbr + ii] -= same2 * x*(1+lambdaR) + rho*relationVecUpdate[lastbr + ii];
                    relationVecUpdate[lastbr + ii] = same2 * x*(1+lambdaR) + rho*relationVecUpdate[lastbr + ii];
                    relationCVec[lastbrc + ii] -= same2 * x*(1+lambdaC) + rho*relationCVecUpdate[lastbrc + ii];
                ...
                for (INT ii=0; ii < dimensionR; ii++) {
                    ...
                    if (x > 0)
                        ...
                        x = -belta2 * alpha*0.1;
                    for (INT jj = 0; jj < dimension; jj++) {
                        matrixDao[lastMa + jj] -=  x * (entityVec[last_head + jj] - entityVec[lastCls + jj]) + rho*matrixUpdate[lastMa+jj];
                        matrixUpdate[lastMa+jj] = x * (entityVec[last_head + jj] - entityVec[lastCls + jj]) + rho*matrixUpdate[lastMa+jj];
                        entityVecDao[last_head + jj] -= x * matrix[lastMa + jj] + rho*entityVecUpdate[last_head +jj];
                        entityVecUpdate[last_head + jj] = x * matrix[lastMa + jj] + rho*entityVecUpdate[last_head +jj];
                        entityVecDao[lastCls + jj] += x * matrix[lastMa + jj] + rho*entityVecUpdate[lastCls +jj];
                        entityVecUpdate[lastCls + jj] = x * matrix[lastMa + jj] + rho*entityVecUpdate[lastCls +jj];
                    }
                    relationVecDao[lastbr + ii] -= same2 * x*(1+lambdaR) + rho*relationVecUpdate[lastbr + ii];
